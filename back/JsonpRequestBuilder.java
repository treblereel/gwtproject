 ...
 *
 * <p>This will result on the client to call the corresponding {@link
 * AsyncCallback#onSuccess(Object)} method.
 *
 * <p>If needed, errors can be handled by a separate callback:
 *
...
public class JsonpRequestBuilder {
  ...
  /**
   * Returns the name of the callback url parameter to send to the server. The default value is
   * "callback".
   */
  ...
  /**
   * Returns the name of the failure callback url parameter to send to the server. The default is
   * null.
   */
  ...
  private <T> JsonpRequest<T> send(
    ...
    if (predeterminedId != null) {
      ...
      request =
          new JsonpRequest<T>(
              callback, timeout, expectInteger, callbackParam, failureCallbackParam);
    }
    request.send(url);
    return request;
  }
