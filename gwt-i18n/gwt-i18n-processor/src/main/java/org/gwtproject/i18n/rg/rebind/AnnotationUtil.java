...
public class AnnotationUtil {
   ...
   *
   * <p>Note that i18n annotations may appear on classes as well as interfaces (a concrete
   * implementation can be supplied rather than just an interface and this is the normal way of
   * using generic Localizable interfaces), so we have to search the super chain as well as other
   * interfaces.
   *
   * <p>The super chain is walked first, so if an ancestor superclass has the requested annotation,
   * it will be preferred over a directly implemented interface.
   *
   ...
   */
  static <T extends Annotation> T getClassAnnotation(
      Types types, TypeElement clazz, Class<T> annotationClass) {
    if (clazz == null) {
      return null;
    }
    ...
    if (annot == null) {
      List<TypeMirror> list =
          types.directSupertypes(clazz.asType()).stream().collect(Collectors.toList());
