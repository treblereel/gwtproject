...
public class MessageFormatParser {
  ...
  public static class ArgumentChunk extends TemplateChunk {
    ...
    @Override
    public void accept(TemplateChunkVisitor visitor) throws UnableToCompleteException {
      visitor.visit(this);
    }
     ...
     *
     * @return the argument number or -1 to refer to the right-most plural argument
     */
  ...
  /**
   * Default implementation of TemplateChunkVisitor -- other implementations should extend this if
   * possible to avoid breakage when new TemplateChunk subtypes are added.
   */
  ...
  /**
   * Represents a static argument, which is used to remove markup from translator view without
   * having to supply it at each callsite.
   */
  ...
  public abstract static class TemplateChunk {
    ...
    public abstract void accept(TemplateChunkVisitor visitor) throws UnableToCompleteException;

    /**
     * Returns the string as this chunk would be represented in a MessageFormat template, with any
     * required quoting such that reparsing this value would produce an equivalent (note, not
     * identical) parse.
     *
     * <p>Note that the default implementation may not be sufficient for all subclasses.
     */
    ...
    /**
     * Returns the string as this chunk would be represented in a MessageFormat template, with any
     * quoting removed. Note that this is distinct from toString in that the latter is intend for
     * human consumption.
     */
    ...
    /**
     * Returns the optionally quoted string value of this chunk as represented in a MessgeFormat
     * string.
     *
  ...
  /**
   * Generate a MessageFormat-style string representing the supplied components, properly quoting
   * any special characters in string literal portions.
   *
   * <p>Note that additional quoting may be required depending on how it will be used, such as
   * backslash-escaping double quotes if it will be used in a generated string constant.
   *
        ...
        case '{':
          ...
          if (ch != '}') {
            throw new ParseException(
                "Invalid message format - { not start of valid argument" + template, curPos);
          }
          if (curChunk != null) {
            chunks.add(curChunk);
          }
  ...
  /**
   * Parse any arguments appended to a format. The syntax is: format[:tag[=value][:tag[=value]]...
   * for example: "date:tz=EST:showoffset"
   *
