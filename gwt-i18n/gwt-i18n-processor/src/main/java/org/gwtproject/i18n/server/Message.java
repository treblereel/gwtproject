...
public interface Message extends Comparable<Message>, MessageTranslation {
   ...
   * @throws MessageProcessingException if thrown by the visitor or its sub-visitors
   */
  void accept(MessageVisitor v) throws MessageProcessingException;

  /**
   * Accept a {@link MessageVisitor}, using translations from the requested locale.
   *
   * @param v {@link MessageVisitor} to call
   * @param locale locale to use for translations, or null to use the messages present in the source
   * @throws MessageProcessingException if thrown by the visitor or its sub-visitors
   */
   ...
   *
   * @return -1 if this message is before {@code o}, 0 if they are equal, or 1 if this is message is
   *     after {code o}
   */
  int compareTo(Message o);

  /**
   * Get the list of all possible messages. If there are not alternate message selectors, there will
   * be a single entry with an empty list and the default value.
   *
   * @return a list of all message forms, lexicographically sorted by the alternate forms for each
   *     message
   */
  Iterable<AlternateFormMapping> getAllMessageForms();

  /**
   * Return the requested annotation present on this message, including parents if the annotation is
   * inherited.
   *
   * @param annotClass
  ...
  /**
   * Return the name of the method for this message - this should generally only be used in
   ...
   *
   * @return possibly empty array of indices into the list returned by {@link #getParameters()}
   */
   ...
   *
   * @param locale a locale to get a translation for, or null to retrieve the message in the source
   * @return a non-null {@link MessageTranslation} instance - if locale is null, or no better match
   *     is found, {@code this} must be returned
   */
  MessageTranslation getTranslation(GwtLocale locale);

  /**
   * Check if a specified annotation is present on this message (including via inheritance if the
