...
public interface MessageCatalogFactory {
  ...
  /**
   * Return a {@link Writer} that can be used to write the source of the messages to be translated
   * for this class.
   *
   * @param context
   * @param fileName relative path for output file, though implementations may ignore this value and
   *     use something else, such as an annotation giving a database key, for example
   * @return MessageCatalogWriter instance or null if the output file could not be created
   * @throws MessageProcessingException
   */
  Writer getWriter(Context context, String fileName) throws MessageProcessingException;
}
