...
public class LocalePropertyProviderGenerator implements PropertyProviderGenerator {
  ...
  public String generate(
    ...
    for (ConfigurationProperty configProp : configProperties) {
      ...
      if (LOCALE_QUERYPARAM.equals(name)) {
        localeQueryParam = configProp.getValues().iterator().next();
        if (localeQueryParam != null
            && localeQueryParam.length() != 0
        ...
        localeCookie = configProp.getValues().iterator().next();
        if (localeCookie != null
            && localeCookie.length() != 0
        ...
        localeUseMeta = isTrue(configProp.getValues().iterator().next());
      } else if (LOCALE_USERAGENT.equals(name)) {
        localeUserAgent = isTrue(configProp.getValues().iterator().next());
      } else if (LOCALE_SEARCHORDER.equals(name)) {
        localeSearchOrder = configProp.getValues().iterator().next();
      }
    }
  ...
  protected void generateCookieLookup(TreeLogger logger, SourceWriter body, String cookieName)
    ...
    body.outdent();
    body.println("}");
    body.println("locale.new = cookies.substring(idx + " + (cookieName.length() + 1) + ", end);");
    body.outdent();
    body.println("}");
  }

  /**
   * Generate JS code that takes the value of the "locale.new" variable and finds parent locales
   * until the value is a supported locale or the default locale.
   *
   * @param logger logger to use
   * @param body
   * @throws UnableToCompleteException
  ...
  protected void generateQueryParamLookup(TreeLogger logger, SourceWriter body, String queryParam)
    ...
    body.println("}");
    body.println(
        "locale.new = queryParam.substring(qpStart + " + (queryParam.length() + 1) + ", end);");
    body.outdent();
    body.println("}");
  }
