...
/**
 * Writes GWT-style Java properties files for translation. This catalog format does not support
 * aggregation of messages from multiple interfaces since there is no way to distinguish messages
 * from another interface from those that were from this interface but no longer used. The output
 * file is assumed to be in UTF-8 encoding rather than using the {@code \\uXXXX} escapes.
 */
public class PropertiesFormat implements MessageCatalogFormat {
  ...
  public void write(
    ...
    writeComment(out, "Generated from " + messageInterface.getQualifiedName());
    if (locale != null) {
   ...
   *
   * <p>In addition to the usual quoting, all spaces are backslash-quoted.
   *
   ...
   *
   * <p>In addition to the usual quoting, leading spaces are backslash-quoted.
   *
  ...
  /**
   * Write comments before a line, pulled from the annotations on a given method.
   *
