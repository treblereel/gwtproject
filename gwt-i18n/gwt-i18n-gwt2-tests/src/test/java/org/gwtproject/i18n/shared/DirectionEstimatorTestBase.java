...
public abstract class DirectionEstimatorTestBase extends TestCase {
  ...
  /**
   * Asserts that the estimated direction of a given String matches the expected direction.
   *
  ...
  protected void assertDirectionEstimation(
      ...
      String str,
      boolean isHtml) {
    assertEquals(expectedDirection, directionEstimator.estimateDirection(str, isHtml));
  }

  /**
   * Asserts that the estimated direction of a given String matches the expected direction. The
   * implementation will usually call {@link #assertDirectionEstimation( Direction,
   * DirectionEstimator, String, boolean)} with the {@code DirectionEstimator} object to be tested.
   *
   * @param expectedDirection The expected direction.
   * @param str A String whose direction is estimated.
   * @param isHtml Whether {@code str} is HTML / HTML-escaped.
   */
  protected abstract void assertDirectionEstimation(
      Direction expectedDirection, String str, boolean isHtml);

  /**
   * Operates like {@link #assertDirectionEstimation(Direction, String, boolean)}, but assuming
   * {@code str} is not HTML / HTML-escaped.
   *
   * @param expectedDirection The expected direction.
   * @param str A String whose direction is estimated.
   */
  protected void assertDirectionEstimation(Direction expectedDirection, String str) {
    assertDirectionEstimation(expectedDirection, str, false);
  }

  /**
   * Operates like {@link #assertDirectionEstimation(Direction, String, boolean)}, but assuming
   * {@code str} is HTML / HTML-escaped.
   *
   * @param expectedDirection The expected direction.
   * @param str A String whose direction is estimated.
   */
  protected void assertDirectionEstimationHtml(Direction expectedDirection, String str) {
