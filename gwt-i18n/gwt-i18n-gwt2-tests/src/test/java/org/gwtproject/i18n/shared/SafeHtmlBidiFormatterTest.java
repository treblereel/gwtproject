...
/**
 * Unit tests for {@link SafeHtmlBidiFormatter}. Tests only methods added in {@code
 * SafeHtmlBidiFormatter}, i.e. instantiating and method overloading.
 */
public class SafeHtmlBidiFormatterTest extends TestCase {
  ...
  SafeHtmlBidiFormatter ltrFormatter = SafeHtmlBidiFormatter.getInstance(LTR, false);

  public void testDirAttr() {
    assertEquals("dirAttr(SafeHtml)", "dir=rtl", ltrFormatter.dirAttr(toSafeHtml(he)).asString());
    assertEquals("dirAttr(String)", "dir=rtl", ltrFormatter.dirAttr(he).asString());
  }
  ...
  public void testGetInstance() {
    ...
    assertEquals(DEFAULT, SafeHtmlBidiFormatter.getInstance(DEFAULT).getContextDir());

    // Check alwaysSpan
    assertEquals(true, SafeHtmlBidiFormatter.getInstance(false, true).getAlwaysSpan());
    assertEquals(false, SafeHtmlBidiFormatter.getInstance(false, false).getAlwaysSpan());
  }
  ...
  public void testMarkAfter() {
    ...
    assertEquals(
        "markAfter(SafeHtml)", LRM_STRING, ltrFormatter.markAfter(toSafeHtml(text)).asString());
    assertEquals("markAfter(String)", LRM_STRING, ltrFormatter.markAfter(text).asString());
  }

  public void testSafeHtmlEstimateDirection() {
    assertEquals(LTR, ltrFormatter.estimateDirection(toSafeHtml(he + "<some verbose tag/>")));
  }
  ...
  public void testSpanWrap() {
    ...
    assertEquals(
        "spanWrap(SafeHtml)", baseResult + LRM, ltrFormatter.spanWrap(toSafeHtml(text)).asString());
    assertEquals("spanWrap(String)", baseResult + LRM, ltrFormatter.spanWrap(text).asString());
    assertEquals(
        "spanWrap(SafeHtml, boolean)",
        baseResult,
        ltrFormatter.spanWrap(toSafeHtml(text), false).asString());
    assertEquals(
        "spanWrap(String, boolean)", baseResult, ltrFormatter.spanWrap(text, false).asString());
  }
  ...
  public void testSpanWrapWithKnownDir() {
    ...
    assertEquals(
        "spanWrapWithKnownDir(Direction, SafeHtml)",
        baseResult + LRM,
        ltrFormatter.spanWrapWithKnownDir(RTL, toSafeHtml(text)).asString());
    assertEquals(
        "spanWrapWithKnownDir(Direction, String)",
        baseResult + LRM,
        ltrFormatter.spanWrapWithKnownDir(RTL, text).asString());
    assertEquals(
        "spanWrapWithKnownDir(Direction, SafeHtml, boolean)",
        baseResult,
        ltrFormatter.spanWrapWithKnownDir(RTL, toSafeHtml(text), false).asString());
    assertEquals(
        "spanWrapWithKnownDir(Direction, String, boolean)",
        baseResult,
  ...
  public void testUnicodeWrap() {
    ...
    assertEquals(
        "unicodeWrap(SafeHtml)",
        baseResult + LRM,
        ltrFormatter.unicodeWrap(toSafeHtml(text)).asString());
    assertEquals(
        "unicodeWrap(String)", baseResult + LRM, ltrFormatter.unicodeWrap(text).asString());
    assertEquals(
  ...
  public void testUnicodeWrapWithKnownDir() {
    ...
    assertEquals(
        "unicodeWrap(SafeHtml, boolean)",
        baseResult,
        ltrFormatter.unicodeWrapWithKnownDir(RTL, toSafeHtml(text), false).asString());
    assertEquals(
        "unicodeWrap(String, boolean)",
        baseResult,
