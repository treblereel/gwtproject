...
import org.gwtproject.i18n.client.gen.TestBadKeysFactory;
import org.gwtproject.safehtml.shared.SafeHtml;
import org.gwtproject.safehtml.shared.SafeHtmlBuilder;

/** Test the same things as I18NTest but with a different module which uses different locales. */
public class I18N2Test extends GWTTestCase {
  ...
  public void testAnnotatedMessages() {
    ...
    assertEquals(
        "Two arguments, second and first, inverted", m.invertedArguments("first", "second"));
    ...
    assertEquals(
        "It is 12:01 on 2007 December 1, Saturday", m.getTimeDate(new Date(107, 11, 1, 12, 1, 2)));
    assertEquals("13 widgets", m.pluralWidgetsOther(13));
    //    assertEquals("A widget", m.pluralWidgetsOther(1));
  }

  public void testBadKeys() {
    TestBadKeys test = TestBadKeysFactory.get();
    ...
    assertEquals("logger_org_hibernate_jdbc", test.getString("logger_org_hibernate_jdbc"));
    ...
    assertEquals("maven_checkstyle_properties", test.maven_checkstyle_properties());
    assertEquals("maven_checkstyle_properties", test.getString("maven_checkstyle_properties"));
    ...
    assertEquals("Cursor_MoveDrop_32x32_File", test.Cursor_MoveDrop_32x32_File());
    assertEquals("Cursor_MoveDrop_32x32_File", test.getString("Cursor_MoveDrop_32x32_File"));
  ...
  public void testDynamicTimeZone() {
    ...
    TimeZoneConstants timeZoneData = new TimeZoneConstants_();
  ...
  public void testListWithArray() {
    ...
    assertEquals(
        "The values are 1, 3, 5, 7, and 9,009", m.valuesArray(new int[] {1, 3, 5, 7, 9009}));
    ...
    assertEquals(
        "The values are 1, 3, 5, 7, and 9,009",
  ...
  public void testListWithList() {
    ...
    assertEquals(
        "The values are 1, 3, 5, 7, and 9,009", m.valuesList(Arrays.asList(1, 3, 5, 7, 9009)));

    assertEquals(
        "There are no values", m.valuesListAsSafeHtml(Arrays.<Integer>asList()).asString());
  ...
  public void testListWtihVarArgs() {
    ...
    assertEquals("The values are 1, 3, 5, 7, and 9,009", m.valuesVarArgs(1, 3, 5, 7, 9009));

    assertEquals("There are no names", m.valuesVarArgsAsSafeHtml().asString());
    assertEquals("The name is John", m.valuesVarArgsAsSafeHtml(sh("John")).asString());
    assertEquals(
        "The names are John and Jeff",
        m.valuesVarArgsAsSafeHtml(sh("John"), sh("Jeff")).asString());
    assertEquals(
        "The names are John, Jeff, and Bob",
        m.valuesVarArgsAsSafeHtml(sh("John"), sh("Jeff"), sh("Bob")).asString());
  }

  /**
   * Verifies correct output for multiple, nested selectors, using an enum for gender selection (and
   * SafeHtml output).
   */
  public void testMultiSelectEnum() {
    ...
    // empty list of names
    assertEquals(
        "Nobody liked his message",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
                1,
                Gender.MALE)
            .asString());
    assertEquals(
        "Nobody liked his 2 messages",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
    ...
    names.add("Bob");
    assertEquals(
        "John and Bob liked his message",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John and Bob liked his 2 messages",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John and Bob liked her message",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John and Bob liked her 3 messages",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John and Bob liked their message",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John and Bob liked their 4 messages",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
    ...
    names.add("Alice");
    assertEquals(
        "John, Bob, and one other liked his message",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John, Bob, and one other liked his 2 messages",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John, Bob, and one other liked her message",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John, Bob, and one other liked her 3 messages",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John, Bob, and one other liked their message",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John, Bob, and one other liked their 4 messages",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
    ...
    names.add("Carol");
    assertEquals(
        "John, Bob, and 2 others liked his message",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John, Bob, and 2 others liked his 2 messages",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John, Bob, and 2 others liked her message",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John, Bob, and 2 others liked her 3 messages",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John, Bob, and 2 others liked their message",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
            ...
            .asString());
    assertEquals(
        "John, Bob, and 2 others liked their 4 messages",
        m.multiSelectEnum(
                names,
                names.size() > 0 ? names.get(0) : null,
                names.size() > 1 ? names.get(1) : null,
  ...
  /**
   * Verifies correct output for multiple, nested selectors, using a string for gender selection.
   */
  ...
  /**
   * Verify that nested annotations are looked up with both A$B names and A_B names. Note that $
   * takes precedence and only one file for a given level in the inheritance tree will be used, so
   * A$B_locale will be used and A_B_locale ignored.
   */
  ...
  public void testSpecialPlurals() {
    ...
    assertEquals(
        "John Doe and Betty Smith have reviewed this movie",
    ...
    assertEquals(
        "John Doe has reviewed this movie", m.reviewersAsSafeHtml(1, "John Doe", null).asString());
    assertEquals(
        "John Doe and Betty Smith have reviewed this movie",
  ...
  public void testStaticCurrency() {
    ...
    assertEquals("The total is A$1,001.02", m.australianDollarsAsSafeHtml(1001.02).asString());
  }
