 ...
 *
 * <p>Multiple cell widgets or Columns can share a single Cell instance, but there may be
 * implications for certain stateful Cells. Generally, Cells are stateless flyweights that see the
 * world as row values/keys. If a Column contains duplicate row values/keys, the Cell will not
 * differentiate the value in one row versus another. Similarly, if you use a single Cell instance
 ...
 * original row value/key. The next time the Cell Widget renders that row value/key, the Cell
 * renders the pending value instead. This allows applications to refresh the Cell Widget without
 * clearing out all of the end user's pending changes. In subclass of {@link AbstractEditableCell},
 * the pending state remains until either the original value is updated (a successful commit), or
 * until {@link AbstractEditableCell#clearViewData(Object)} is called (a failed commit).
 *
 * <p>If you share an interactive Cell between two cell widgets (or Columns within the same
 * CellTable), then when the end user updates the pending value in one widget, it will be reflected
 * in the other widget <i>when the other widget is redrawn</i>. You should base your decision on
 * whether or not to share Cell instances on this behavior.
 *
 * <p>
 *
 * <h3>Example</h3>
 ...
 *
 * <p><span style="color:red;">Warning: The Cell interface may change in subtle but breaking ways as
 * we continuously seek to improve performance. You should always subclass {@link AbstractCell}
 * instead of implementing {@link Cell} directly.</span>
 *
...
public interface Cell<C> {
  ...
  class Context {
    ...
    /**
     * Get the column index of the cell. If the view only contains a single column, this method
     * returns 0.
     *
    ...
    /**
     * Get the sub index of the rendered row value. If the row value renders to a single row
     * element, the sub index is 0. If the row value renders to more than one row element, the sub
     * index may be greater than zero.
     */
  ...
  /**
   * Check if this cell handles selection. If the cell handles selection, then its container should
   * not automatically handle selection.
   *
