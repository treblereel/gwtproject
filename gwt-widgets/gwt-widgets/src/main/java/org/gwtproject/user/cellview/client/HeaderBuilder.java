...
/**
 * Builds the DOM elements for the header section of a CellTable. It also provides queries on
 * elements in the last DOM subtree that it created.
 *
 * <p>{@link HeaderBuilder} provides two optional ways to handle events, via a {@link Column}, a
...
public interface HeaderBuilder<T> {
  ...
  /**
   * Builds the DOM subtree for this header. The root of the subtree must be a THEAD element, as
   * appropriate. This method may be called multiple times and should return a new DOM subtree each
   * time.
   *
   * <p>If the header is empty, return null.
   *
   * @return a {@link TableSectionBuilder} representing the new header, or null if the header is
   *     empty
   */
  TableSectionBuilder buildHeader();

  /**
   * Given an element in the DOM subtree returned by the most recent call to {@link #buildHeader()},
   * returns the Column that should be the target of any button clicks or other events on that
   * element, or null if the events should be discarded. The column is used to support features such
   * as column sorting.
   *
   ...
   * to return the appropriate instance and cell table will forward events originating in the
   * element to the {@link Header}. Return null if events from the element should be discarded.
   *
   ...
   * {@link #getHeader(Element)} would return null.
   *
   * @param elem the element of interest
   */
