...
/**
 * Iterator over a finite number of widgets, which are stored in a delegate class (usually a widget
 * panel).
 *
 * <p>In order to use this class, assign each widget in the panel an arbitrary index. For example,
 * {@link HeaderPanel} defines the header as index 0, the content as index 1, and the footer as
 * index 2. Construct a new {@link FiniteWidgetIterator} with a {@link WidgetProvider} that provides
 * the child widgets.
 */
class FiniteWidgetIterator implements Iterator<Widget> {
   ...
   *
   * <p>The widget count is the number of child widgets that the panel supports, regardless of
