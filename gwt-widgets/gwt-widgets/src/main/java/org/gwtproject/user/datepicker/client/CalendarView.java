...
/**
 * The CalendarView is a calendar grid that represents the current view of a {@link DatePicker}.
 * Note, the calendar view only deals with the currently visible dates and all state is flushed when
 * the calendar view is refreshed.
 */
public abstract class CalendarView extends DatePickerComponent {
  ...
  /**
   * Adds a style name to the cell of the supplied date. This style is only set until the next time
   * the {@link CalendarView} is refreshed.
   *
   ...
   *
   * @param date the date of the cell where aria-selected should be set, or null to clear
   *     aria-selected.
   */
  public void setAriaSelectedCell(Date date) {}

  /**
   * Enables or Disables a particular date. by default all valid dates are enabled after a rendering
   * event. Disabled dates cannot be selected.
   *
