 ...
 *
 * <p>An {@link IsRenderable} version of {@link HTMLPanel}. This class is a stepping in our
 * transition to the Renderable strategy. Eventually this functionality should be merged into {@link
 * HTMLPanel}. The only reason this class doesn't extend {@link HTMLPanel} is because it doesn't
 * provide any way to build the panel lazily (which is needed here).
 */
public class RenderablePanel extends ComplexPanel implements IsRenderable {
   ...
   * this HTML that has a specified id can contain a child widget. The actual element that will hold
   * this HTML isn't initialized until it is needed.
   *
  ...
  /**
   * Adopts the given, but doesn't change anything about its DOM element. Should only be used for
   * widgets with elements that are children of this panel's element. No-op if called with an {@link
   * IsRenderable} that isn't also IsWidget, but safe to call with such as a convenience.
   */
  ...
  /**
   * Whether the initilization of the panel is finished (i.e., the corresponding DOM element has
   * been built).
   */
  ...
  /**
   * Method that finishes the initialization of HTMLPanel instances built from HTML. This will
   * create a div to wrap the given HTML and call any callbacks that may have been added to the
   * panel.
   */
