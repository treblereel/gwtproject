 ...
 * body (i.e. {@link RootPanel#get()}).
 *
 * <p>This panel automatically calls {@link RequiresResize#onResize()} on itself when initially
 * created, and whenever the window is resized.
 *
 * <p>NOTE: This widget will <em>only</em> work in standards mode, which requires that the HTML page
...
public class RootLayoutPanel extends LayoutPanel {
  ...
  /**
   * Gets the singleton instance of RootLayoutPanel. This instance will always be attached to the
   * document body via {@link RootPanel#get()}.
   *
   * <p>Note that, unlike {@link RootPanel#get(String)}, this class provides no way to get an
   * instance for any element on the page other than the document body. This is because we know of
   * no way to get resize events for anything but the window.
   */
