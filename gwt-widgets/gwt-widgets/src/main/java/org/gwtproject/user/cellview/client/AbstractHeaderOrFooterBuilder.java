...
public abstract class AbstractHeaderOrFooterBuilder<T>
  ...
  /** A map that provides O(1) access to a value given the key, or to the key given the value. */
  ...
  /**
   * Check if the icon is located at the start or end of the line. The start of the line refers to
   * the left side in LTR mode and the right side in RTL mode. The default location is the start of
   * the line.
   */
  ...
  /**
   * Set the position of the sort icon to the start or end of the line. The start of the line refers
   * to the left side in LTR mode and the right side in RTL mode. The default location is the start
   * of the line.
   */
  ...
  /**
   * Enables column-specific event handling for the specified element. If a column is sortable, then
   * clicking on the element or a child of the element will trigger a sort event.
   *
   * @param builder the builder to associate with the column. The builder should be a child element
   *     of a row returned by {@link #startRow} and must be in a state where an attribute can be
   *     added.
  ...
  /**
   * Renders a given Header into a given ElementBuilderBase. This method ensures that the CellTable
   * widget will handle events events originating in the Header.
   *
   * @param <H> the data type of the header
   * @param out the {@link ElementBuilderBase} to render into. The builder should be a child element
   *     of a row returned by {@link #startRow} and must be in a state that allows both attributes
   *     and elements to be added
   * @param context the {@link Context} of the header being rendered
   * @param header the {@link Header} to render
   */
  protected final <H> void renderHeader(
      ElementBuilderBase<?> out, Context context, Header<H> header) {
   ...
   */
  protected final void renderSortableHeader(
      ElementBuilderBase<?> out,
      Context context,
      Header<?> header,
      boolean isSorted,
  ...
  /**
   * Get the HTML representation of the sort icon. These are loaded lazily so the compiler has a
   * chance to strip this method, and the icon source code, if the user overrides renderHeader.
   *
