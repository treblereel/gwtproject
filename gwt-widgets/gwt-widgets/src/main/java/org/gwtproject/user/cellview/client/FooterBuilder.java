...
public interface FooterBuilder<T> {
  ...
  /**
   * Builds the DOM subtree for this footer. The root of the subtree must be a TFOOT element, as
   * appropriate. This method may be called multiple times and should return a new DOM subtree each
   * time.
   *
   * <p>If the footer is empty, return null.
   *
   * @return a {@link TableSectionBuilder} representing the new footer, or null if the footer is
   *     empty
   */
  TableSectionBuilder buildFooter();

  /**
   * Given an element in the DOM subtree returned by the most recent call to {@link #buildFooter()},
