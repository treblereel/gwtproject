 ...
 *
 * <p>Because some browsers do not support rich text editing, and others support only a limited
 * subset of functionality, there are two formatter interfaces, accessed via {@link
 * #getBasicFormatter()} and {@link #getExtendedFormatter()} . A browser that does not support rich
 * text editing at all will return <code>null</code> for both of these, while one that supports only
 * the basic functionality will return <code>null</code> for the latter.
 *
 * <p><img class='gallery' src='doc-files/RichTextArea.png'/>
 *
 * <h3>CSS Style Rules</h3>
 *
 * <dl>
 *   <dt>.gwt-RichTextArea
 *   <dd>Applied to the rich text element.
 * </dl>
 */
public class RichTextArea extends FocusWidget
    implements HasHTML, HasInitializeHandlers, HasSafeHtml {

  /**
   * This interface is used to access basic formatting options, when available. If the
   * implementation supports basic formatting, then {@link RichTextArea#getBasicFormatter()} will
   * return an instance of this class.
   *
   * <p>The formatter will format the user selected text in the {@link RichTextArea}. As a result,
   * it will only work reliably if the {@link RichTextArea} is attached, visible to on the page, and
   * has been focused at least once. If you just want to initialize the content of the {@link
   * RichTextArea}, use {@link RichTextArea#setHTML(String)} instead.
   *
  ...
  /**
   * This interface is used to access full formatting options, when available. If the implementation
   * supports full formatting, then {@link RichTextArea#getExtendedFormatter()} will return an
  ...
  /**
   * This interface is used to access full formatting options, when available. If the implementation
   * supports full formatting, then {@link RichTextArea#getFormatter()} will return an instance of
   * this class.
   *
   * <p>The formatter will format the user selected text in the {@link RichTextArea}. As a result,
   * it will only work reliably if the {@link RichTextArea} is attached, visible to on the page, and
  ...
  /**
   * Justification enumeration. The three values are <code>left</code>, <code>right</code>, <code>
   * center</code>.
   */
  ...
  /**
   * Gets the basic rich text formatting interface. Note that formatting can only be done when the
   * {@link RichTextArea} is attached, visible on the page, and has been focused by the user.
   *
  ...
  /**
   * Gets the full rich text formatting interface. Note that formatting can only be done when the
   * {@link RichTextArea} is attached, visible on the page, and has been focused by the user.
   *
  ...
  /**
   * Gets the rich text formatting interface. Note that formatting can only be done when the {@link
   * RichTextArea} is attached, visible on the page, and has been focused by the user.
   *
   * @return <code>null</code> if full formatting is not supported
   */
