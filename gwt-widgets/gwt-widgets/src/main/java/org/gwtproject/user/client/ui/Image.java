 ...
 *
 * <dl>
 *   <dt>.gwt-Image The outer element
 * </dl>
 *
 * <p>Tranformations between clipped and unclipped state will result in a loss of any style names
 * that were set/added; the only style names that are preserved are those that are mentioned in the
...
public class Image extends Widget
  ...
  /**
   * The attribute that is set when an image fires a native load or error event before it is
   * attached.
   */
  ...
  public Image(ImageResource resource) {
    if (resource instanceof ImageResourcePrototype.Bundle) {
      ...
      changeState(
          new UnclippedState(
              this, resource.getSafeUri(), resource.getWidth(), resource.getHeight()));
    }
    setStyleName("gwt-Image");
  }

  /**
   * Creates an image with a specified URL. The load event will be fired once the image at the given
   * URL has been retrieved by the browser.
   *
  ...
  /**
   * Creates a clipped image with a specified URL and visibility rectangle. The visibility rectangle
   * is declared relative to the rectangle which encompasses the entire image, which has an
   * upper-left vertex of (0,0). The load event will be fired immediately after the object has been
   * constructed (i.e. potentially before the image has been loaded in the browser). Since the width
   * and height are specified explicitly by the user, this behavior will not cause problems with
   * retrieving the width and height of a clipped image in a load event handler.
   *
   * @param url the URL of the image to be displayed
  ...
  /**
   * Gets the height of the image. When the image is in the unclipped state, the height of the image
   * is not known until the image has been loaded (i.e. load event has been fired for the image).
   *
   ...
   * If the image is in the unclipped state, then the visibility rectangle is assumed to be the
   * rectangle which encompasses the entire image, which has an upper-left vertex of (0,0).
   *
   * @return the horizontal co-ordinate of the upper-left vertex of the image's visibility rectangle
   */
  ...
  /**
   * Gets the vertical co-ordinate of the upper-left vertex of the image's visibility rectangle. If
   * the image is in the unclipped state, then the visibility rectangle is assumed to be the
   * rectangle which encompasses the entire image, which has an upper-left vertex of (0,0).
   *
   * @return the vertical co-ordinate of the upper-left vertex of the image's visibility rectangle
   */
  ...
  /**
   * Gets the URL of the image. The URL that is returned is not necessarily the URL that was passed
   * in by the user. It may have been transformed to an absolute URL.
   *
  ...
  /**
   * Sets the URL of the image to be displayed. If the image is in the clipped state, a call to this
   * method will cause a transition of the image to the unclipped state. Regardless of whether or
   * not the image is in the clipped or unclipped state, a load event will be fired.
   *
  ...
  /**
   * Gets the width of the image. When the image is in the unclipped state, the width of the image
   * is not known until the image has been loaded (i.e. load event has been fired for the image).
   *
  ...
  /**
   * Sets the url and the visibility rectangle for the image at the same time, based on an
   * ImageResource instance. A single load event will be fired if either the incoming url or
   * visiblity rectangle co-ordinates differ from the image's current url or current visibility
   * rectangle co-ordinates. If the image is currently in the unclipped state, a call to this method
   * will cause a transition to the clipped state.
   *
   * @param resource the ImageResource to display
   */
  public void setResource(ImageResource resource) {
    if (resource instanceof ImageResourcePrototype.Bundle) {
      state.setUrlAndVisibleRect(
          this,
          resource.getSafeUri(),
          resource.getLeft(),
          resource.getTop(),
  ...
  /**
   * Sets the url and the visibility rectangle for the image at the same time. A single load event
   * will be fired if either the incoming url or visiblity rectangle co-ordinates differ from the
   * image's current url or current visibility rectangle co-ordinates. If the image is currently in
   * the unclipped state, a call to this method will cause a transition to the clipped state.
   *
   * @param url the image URL
   * @param left the horizontal coordinate of the upper-left vertex of the visibility rectangle
   * @param top the vertical coordinate of the upper-left vertex of the visibility rectangle
   * @param width the width of the visibility rectangle
   * @param height the height of the visibility rectangle
   */
  ...
  /**
   * Sets the visibility rectangle of an image. The visibility rectangle is declared relative to the
   * rectangle which encompasses the entire image, which has an upper-left vertex of (0,0). Provided
   * that any of the left, top, width, and height parameters are different than the those values
   * that are currently set for the image, a load event will be fired. If the image is in the
   * unclipped state, a call to this method will cause a transition of the image to the clipped
   * state. This transition will cause a load event to fire.
   *
   * @param left the horizontal coordinate of the upper-left vertex of the visibility rectangle
   * @param top the vertical coordinate of the upper-left vertex of the visibility rectangle
   * @param width the width of the visibility rectangle
  ...
  private static class ClippedState extends State {
    ...
    public void setUrlAndVisibleRect(
        Image image, SafeUri url, int left, int top, int width, int height) {
      /*
  ...
  /** Abstract class which is used to hold the state associated with an image object. */
  abstract static class State {
    ...
    /**
     * Called when the widget is attached to the page. Not to be confused with the load event that
    ...
    public abstract void setUrlAndVisibleRect(
        Image image, SafeUri url, int left, int top, int width, int height);

    public abstract void setVisibleRect(Image image, int left, int top, int width, int height);

    /**
     * We need to synthesize a load event in case the image loads synchronously, before our handlers
     * can be attached.
     *
