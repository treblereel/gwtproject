 ...
 *
 * <p>Many of the tests in this class can run even in HtmlUnit and browsers that do not support
 * touch events because we create mock touch events.
 */
@J2clTestInput(TouchScrollTest.class)
public class TouchScrollTest extends GWTTestCase {
   ...
   */
  private static NativeEvent createNativeTouchEvent() {
    JsArray touches = new JsArray();
    JsPropertyMap result = JsPropertyMap.of();
    result.set("changedTouches", touches);
    result.set("targetTouches", touches);
    result.set("touches", touches);
    result.set("preventDefault", (Fn) () -> {});
   ...
   * @return a mock touch
   */
  private static Touch createTouch(int x, int y) {
    JsPropertyMap touch = JsPropertyMap.of();
    touch.set("clientX", x);
    touch.set("clientY", y);
    touch.set("identifier", 0);
    touch.set("pageX", x);
    touch.set("pageY", y);
    touch.set("screenX", x);
    touch.set("screenY", y);
    touch.set("target", null);
    return Js.uncheckedCast(touch);
  }
  ...
  /** Test that the bust click/attach event handler is removed from the old widget. */
  ...
  /**
   * Test that when momentum ends, the momentum command is set to null (and isMomentumActive()
   * returns false).
   */
  public void testMomentumEnd() {
    // Use a short lived momentum.
    scroller.setMomentum(
        new DefaultMomentum() {
          @Override
  ...
  /** Test that we handle touch end events that occur without initiating a drag sequence. */
  ...
  /** Test that the setupBustClickHandler is called when the widget is detached and re-attached. */
  ...
  /**
   * A replacement for JUnit's {@link #tearDown()} method. This method runs once per test method in
   * your subclass, just after your each test method runs and can be used to perform cleanup.
   * Override this method instead of {@link #tearDown()}. This method is run even in pure Java mode
   * (non-GWT).
   *
  ...
  private RepeatingCommand getMomentumCommand(TouchScroller scroller) {
    return scroller.momentumCommand;
  }

  /**
   * Test that {@link TouchScroller} defers to native scrolling if the scrollable widget is already
   * scrolled as far as it can go.
   *
  ...
  /**
   * A custom {@link ScrollPanel} that doesn't rely on the DOM to calculate its vertical and
   * horizontal position. Allows testing in HtmlUnit.
   */
  private static class CustomScrollPanel extends ScrollPanel {
    ...
    /**
     * Construct a new {@link CustomScrollPanel} using 0 as the minimum vertical and horizontal
     * scroll position and INTEGER.MAX_VALUE as the maximum positions.
     */
