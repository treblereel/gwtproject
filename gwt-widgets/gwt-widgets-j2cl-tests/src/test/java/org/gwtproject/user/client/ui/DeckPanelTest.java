...
public class DeckPanelTest extends PanelTestBase<DeckPanel> {
  ...
  /**
   * Test that the {@link DeckPanel} calls widget.setVisible(true) on the visible widget, but does
   * NOT call widget.setVisible(false) when a widget is hidden.
   */
  ...
  /**
   * Test that the offsetHeight/Width of a widget are defined when the widget is added to the
   * DeckPanel.
   */
    ...
    Label content =
        new Label("detached") {
          ...
          public void onLoad() {
            ...
            assertTrue(
                "Expect positive offsetWidth. " + "This will fail in WebKit if run headless",
