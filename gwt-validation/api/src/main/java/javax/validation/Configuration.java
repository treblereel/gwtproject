...
public interface Configuration<T extends Configuration<T>> {
  ...
  /**
   * Ignore data from the <i>META-INF/validation.xml</i> file if this method is called. This method
   * is typically useful for containers that parse <i>META-INF/validation.xml</i> themselves and
   * pass the information via the <code>Configuration</code> methods.
   *
   ...
   * interpolator. If <code>null</code> is passed, the default message interpolator is used (defined
   * in XML or the specification default).
   *
   ...
   * resolver. If <code>null</code> is passed, the default traversable resolver is used (defined in
   * XML or the specification default).
   *
   ...
   * factory. If null is passed, the default constraint validator factory is used (defined in XML or
   ...
   *
   * <p>The stream should be closed by the client API after the <code>ValidatorFactory</code> has
   * been built. The Bean Validation provider must not close the stream.
   *
  ...
  /**
   * Return an implementation of the <code>MessageInterpolator</code> interface following the
   * default <code>MessageInterpolator</code> defined in the specification:
   *
   * <ul>
   *   <li>use the ValidationMessages resource bundle to load keys
   *   <li>use Locale.getDefault()
   * </ul>
   *
  ...
  /**
   * Return an implementation of the <code>TraversableResolver</code> interface following the
   * default <code>TraversableResolver</code> defined in the specification:
   *
   * <ul>
   *   <li>if Java Persistence is available in the runtime environment, a property is considered
   *       reachable if Java Persistence considers the property as loaded
   *   <li>if Java Persistence is not available in the runtime environment, all properties are
   *       considered reachable
   *   <li>all properties are considered cascadable.
   * </ul>
   *
  ...
  /**
   * Return an implementation of the <code>ConstraintValidatorFactory</code> interface following the
   * default <code>ConstraintValidatorFactory</code> defined in the specification:
   *
   * <ul>
   *   <li>uses the public no-arg constructor of the <code>ConstraintValidator</code>
   * </ul>
   *
