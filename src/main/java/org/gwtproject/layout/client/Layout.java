 ...
 *
 * <p>
 * This class is typically used by higher-level widgets to implement layout on their behalf. It is
 * intended to wrap an element (usually a &lt;div&gt;), and lay its children out in a predictable
 * fashion, automatically accounting for changes to the parent's size, and for all elements'
 * margins, borders, and padding.
 * </p>
 *
 * <p>
 * To use this class, create a container element (again, usually a &lt;div&gt;) and pass it to
 ...
 *
 * <p>
 * A separate {@link Layer} instance is associated with each child element. There is a set of
 * methods available on this class to manipulate the child element's position and size. In order for
 * changes to a layer to take effect, you must finally call one of {@link #layout()} or {@link
 * #layout(int)}. This allows many changes to different layers to be applied efficiently, and to be
 * animated.
 * </p>
 *
 * <p>
 * On most browsers, this is implemented using absolute positioning. It also contains extra logic to
 * make IE6 work properly.
 * </p>
 *
 * <p>
 * <h3>Example</h3>
 ...
 *
 * <p>
 * NOTE: This class will <em>only</em> work in standards mode, which requires that the HTML page in
 * which it is run have an explicit &lt;!DOCTYPE&gt; declaration.
 * </p>
 *
 * <p>
 * NOTE: This class is still very new, and its interface may change without warning. Use at your own
 * risk.
 * </p>
 */
public class Layout {
   ...
   *
   * <p>
   * This method will attach the child to the layout, removing it from its current parent element.
   ...
   *
   * <p>
   * This is most useful for top-level layouts that need to follow the size of another element, such
   * as the &lt;body&gt;.
   * </p>
   */
   ...
   *
   * <p>
   * This will work for any unit type, but be aware that certain unit types, such as {@link
   * Unit#EM}, and {@link Unit#EX}, will return different values based upon the parent's associated
   * font size. {@link Unit#PCT} is dependent upon the parent's actual size, and the axis to be
   * measured.
   * </p>
   *
  ...
  /**
   * Updates this layout's children immediately. This method <em>must</em> be called after updating
   * any of its children's {@link Layer layers}.
   */
  public void layout() {
    layout(0);
  }
  ...
  /**
   * Updates the layout by animating it over time, with a callback on each frame of the animation,
   * and upon completion.
   *
   * @param duration the duration of the animation
  ...
  /**
   * Callback interface used by {@link Layout#layout(int, AnimationCallback)} to provide updates on
   * animation progress.
   */
  public interface AnimationCallback {

    /**
     * Called immediately after the animation is complete, and the entire layout is in its final
     * state.
     */
   ...
   *
   * <p>
   * Each child element has three values associated with each axis: {left, right, width} on the
   * horizontal axis, and {top, bottom, height} on the vertical axis. Precisely two of three values
   * may be set at a time, or the system will be over- or under-contrained. For this reason, the
   * following methods are provided for setting these values:
   * <ul>
   * <li>{@link Layer#setLeftRight}</li>
   * <li>{@link Layer#setLeftWidth}</li>
   * <li>{@link Layer#setRightWidth}</li>
   * <li>{@link Layer#setTopBottom}</li>
   * <li>{@link Layer#setTopHeight}</li>
   * <li>{@link Layer#setBottomHeight}</li>
   * </ul>
   * </p>
   *
   * <p>
   * By default, each layer is set to fill the entire parent (i.e., {left, top, right, bottom} = {0,
   * 0, 0, 0}).
   * </p>
   */
  public class Layer {
     ...
     *
     * <p>
     * This is the element that sits between the parent and child elements. It is normally necessary
     * to operate on this element only when you need to modify CSS properties that are not directly
     * modeled by the Layer class.
     * </p>
     *
