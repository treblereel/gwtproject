...
public class JsonpRequest<T> {
  ...
  /**
   * Prefix appended to all id's that are passed in by the user. The "P" suffix must stay in sync
   * with ExternalTextResourceGenerator.java
   */
  private static final String PREDETERMINED_ID_PREFIX = "P";

  /** Returns the next ID to use, incrementing the global counter. */
  // TODO export to JS -> massive Abweidhungen vom Original
  ...
  /**
   * Returns a global object to store callbacks of pending requests, creating it if it doesn't
   * exist.
   */
  // TODO export to JS -> massive Abweidhungen vom Original
   ...
   * @param timeout Time in ms after which a {@link TimeoutException} will be thrown
   * @param expectInteger Should be true if T is {@link Integer}, false otherwise
   * @param callbackParam Name of the url param of the callback function name
   * @param failureCallbackParam Name of the url param containing the failure callback function
   *     name, or null for no failure callback
   */
  JsonpRequest(
  ...
  /**
   * Create a new JSONP request with a hardcoded id. This could be used to manually control which
   * resources are considered duplicates (by giving them identical ids). Could also be used if the
   * callback name needs to be completely user controlled (since the id is part of the callback
   * name).
   *
   * @param callback The callback instance to notify when the response comes back
   * @param timeout Time in ms after which a {@link TimeoutException} will be thrown
  ...
  /**
   * Cancels a pending request. Note that if you are using preset ID's, this will not work, since
   * there is no way of knowing if there are other requests pending (or have already returned) for
   * the same data.
   */
  public void cancel() {
    timer.cancel();
  ...
  /**
   * Cleans everything once the response has been received: deletes the script tag and unregisters
   * the callback.
   */
  private void unload() {
    /*
