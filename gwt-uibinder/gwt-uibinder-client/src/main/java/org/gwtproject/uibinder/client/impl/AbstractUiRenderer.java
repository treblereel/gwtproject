...
public abstract class AbstractUiRenderer implements UiRenderer {
  ...
  protected abstract static class UiRendererDispatcher<T> implements HasHandlers {
    ...
    /**
     * Maps strings describing event types and field names to methods contained in type {@code T}
     * (which are indexed by an integer).
     */
   ...
   * @throws IllegalArgumentException if the {@code parent} does not point to or contains a
   *     previously rendered element. In DevMode also when the root element is not attached to the
   *     DOM
   * @throws IllegalStateException parent does not contain an element matching {@code filedName}
  ...
  protected static Element findInnerField(Element parent, String fieldName, String attribute) {
    ...
    if (elementById == null) {
      if (!isAttachedToDom(root)) {
        ...
        throw new IllegalArgumentException(
            "UiRendered element is not attached to DOM, or \""
                + fieldName
  ...
  /**
   * Retrieves the root of a previously rendered element contained within the {@code parent}. The
   * {@code parent} must either contain the previously rendered DOM structure as its only child, or
   * point directly to the rendered element root.
   *
  ...
  /**
   * Inserts an attribute into the first tag found in a {@code safeHtml} template. This method
   * assumes that the {@code safeHtml} template begins with an open HTML tag. {@code SafeHtml}
   * templates produced by UiBinder always meet these conditions.
   *
   * <p>This method does not attempt to ensure {@code atributeName} and {@code attributeValue}
   * contain safe values.
   *
   ...
   */
  protected static SafeHtml stampUiRendererAttribute(
      SafeHtml safeHtml, String attributeName, String attributeValue) {
    ...
    html =
        html.substring(0, endOfFirstTag)
            + " "
            + attributeName
            + "=\""
            + attributeValue
   ...
   */
  private static int computeDispatchEvent(
      HashMap<String, Integer> table, Element root, NativeEvent event) {
   ...
   * @param parent element containing, or pointing to, a previously rendered DOM structure
   * @param attribute attribute name that identifies the root of the DOM structure
   * @return the root element of the previously rendered DOM structure or <code>null</code> if
   *     {@code parent} does not contain a previously rendered element
   * @throws NullPointerException if {@code parent} == null
   */
   ...
   * @return the field name or {@code null} if the {@code element} does not have an id attribute as
   *     would be produced by {@link #buildInnerId(String, String)}) with {@code fieldName} and
   *     {@code uiId}
   */
