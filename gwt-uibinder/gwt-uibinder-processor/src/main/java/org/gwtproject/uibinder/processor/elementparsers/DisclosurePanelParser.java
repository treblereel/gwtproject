...
public class DisclosurePanelParser implements ElementParser {
  ...
  public void parse(
      final XMLElement panelElem, String panelField, TypeMirror type, final UiBinderWriter writer)
    ...
    if (children.header != null) {
      ...
      String headerText =
          children.header.consumeInnerTextEscapedAsHtmlStringLiteral(new TextInterpreter(writer));

      if (openImage == null ^ closedImage == null) {
        writer.die(
            children.header,
            "Both %s and %s must be specified, or neither",
            OPEN_IMAGE,
            CLOSED_IMAGE);
      }
      ...
      if (openImage != null) {
        writer.setFieldInitializer(
            panelField,
            String.format(
                "new %s(%s, %s, \"%s\")", panelTypeName, openImage, closedImage, headerText));
      } else {
        writer.setFieldInitializer(
            panelField, String.format("new %s(\"%s\")", panelTypeName, headerText));
      }
    }
  }
