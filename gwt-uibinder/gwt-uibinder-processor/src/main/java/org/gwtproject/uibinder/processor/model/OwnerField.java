...
public class OwnerField {
  ...
  public OwnerField(VariableElement field, MortalLogger logger, UiBinderContext context)
    ...
    if (fieldTypeMirror == null) {
      logger.die("Type for field " + name + " is not a class: " + field.asType());
    }
    ...
    AnnotationMirror annotation =
        AptUtil.getAnnotation(field, UiBinderApiPackage.current().getUiFieldFqn());

    if (annotation == null) {
      logger.die("Field " + name + " is not annotated with @UiField");
    }

    AnnotationValue provided = AptUtil.getAnnotationValues(annotation).get("provided");
