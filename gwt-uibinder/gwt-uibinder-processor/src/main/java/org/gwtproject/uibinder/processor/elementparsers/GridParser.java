...
public class GridParser implements ElementParser {
  ...
  public void parse(XMLElement elem, String fieldName, TypeMirror type, UiBinderWriter writer)
    ...
    if (size.getRows() > 0 || size.getColumns() > 0) {
      writer.addStatement(
          "%s.resize(%s, %s);",
          fieldName, Integer.toString(size.getRows()), Integer.toString(size.getColumns()));

      for (RowContent row : matrix) {
        if ((row.getStyleName() != null) && (!row.getStyleName().isEmpty())) {
          writer.addStatement(
              "%s.getRowFormatter().setStyleName(%s, %s);",
              fieldName, matrix.indexOf(row), row.getStyleName());
        }
        ...
        for (CellContent column : row.getColumns()) {
          ...
          if ((column.getStyleName() != null) && (!column.getStyleName().isEmpty())) {
            writer.addStatement(
                "%s.getCellFormatter().setStyleName(%s, %s, %s);",
                fieldName,
                matrix.indexOf(row),
  ...
  private void parseColumns(
      String fieldName, UiBinderWriter writer, RowContent row, XMLElement child)
      throws UnableToCompleteException {
    ...
    for (XMLElement cell : child.consumeChildElements()) {
      ...
      if (!tagName.equals(CELL_TAG) && !tagName.equals(CUSTOMCELL_TAG)
        ...
        writer.die(
            "Grid's row tag in %s may only contain %s or %s element.",
  ...
  private void parseRows(
      XMLElement elem, String fieldName, UiBinderWriter writer, List<RowContent> matrix)
      throws UnableToCompleteException {
