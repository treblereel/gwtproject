...
public class CellPanelParser implements ElementParser {
   ...
   *
   * <p>This is exposed publicly because there is a DockPanelParser that overrides the default
   * behavior, but still needs to parse these attributes.
   */
  protected static void parseCellAttributes(
      XMLElement cellElem, String fieldName, FieldWriter childField, UiBinderWriter writer)
      throws UnableToCompleteException {
    TypeElement hAlignConstantType =
        AptUtil.getElementUtils()
            .getTypeElement(UiBinderApiPackage.current().getHorizontalAlignmentConstantFqn());
    TypeElement vAlignConstantType =
        AptUtil.getElementUtils()
    ...
    if (cellElem.hasAttribute(HALIGN_ATTR)) {
      ...
      writer.addStatement(
          "%1$s.setCellHorizontalAlignment(%2$s, %3$s);",
    ...
    if (cellElem.hasAttribute(VALIGN_ATTR)) {
      ...
      writer.addStatement(
          "%1$s.setCellVerticalAlignment(%2$s, %3$s);",
          fieldName, childField.getNextReference(), value);
    }
    ...
    if (cellElem.hasAttribute(WIDTH_ATTR)) {
      ...
      writer.addStatement(
          "%1$s.setCellWidth(%2$s, %3$s);", fieldName, childField.getNextReference(), value);
    }

    if (cellElem.hasAttribute(HEIGHT_ATTR)) {
      String value = cellElem.consumeStringAttribute(HEIGHT_ATTR);
      writer.addStatement(
          "%1$s.setCellHeight(%2$s, %3$s);", fieldName, childField.getNextReference(), value);
    }
  }
    ...
    for (XMLElement child : elem.consumeChildElements()) {
      ...
      if (ns != null && ns.equals(elem.getNamespaceUri()) && localTagNameIsCell(tagName)) {
        ...
        if (!writer.isWidgetElement(child)) {
          writer.die(
              elem,
              "Expected a widget or <%s:%s>, found %s",
              elem.getPrefix(),
              CELL_TAG.toLowerCase(Locale.ROOT),
              child);
        }
