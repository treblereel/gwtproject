...
public class CustomButtonParser implements ElementParser {
  ...
  public void parse(
      final XMLElement elem, final String fieldName, TypeMirror type, final UiBinderWriter writer)
          ...
          public Boolean interpretElement(XMLElement child) throws UnableToCompleteException {
            ...
            if (innerHtml.length() > 0) {
              writer.addStatement(
                  ...
                  writer.declareTemplateCall(innerHtml, fieldName));
            }

            if (child.hasAttribute("image")) {
              String image = child.consumeImageResourceAttribute("image");
              writer.addStatement(
                  "%s.%s().setImage(new %s(%s));",
                  fieldName,
                  faceNameGetter(faceName),
                  UiBinderApiPackage.current().getImageFqn(),
                  image);
            }
