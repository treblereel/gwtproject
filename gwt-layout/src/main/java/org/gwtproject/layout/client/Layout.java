 ...
 *
 * <p>This class is typically used by higher-level widgets to implement layout on their behalf. It
 * is intended to wrap an element (usually a &lt;div&gt;), and lay its children out in a predictable
 ...
 *
 * <p>NOTE: This class is still very new, and its interface may change without warning. Use at your
 * own risk.
 */
public class Layout {
   ...
   *
   * <p>This method will attach the child to the layout, removing it from its current parent
   * element. Use the {@link Layer} it returns to manipulate the child.
   *
   ...
   *
   * <p>This is most useful for top-level layouts that need to follow the size of another element,
   * such as the &lt;body&gt;.
   */
   ...
   *
   * <p>This will work for any unit type, but be aware that certain unit types, such as {@link
  ...
  private void adjustVerticalConstraints(int parentHeight, Layer l) {
    ...
    if (l.setTop && !l.setTargetTop) {
      ...
      if (!l.setHeight) {
        ...
        l.sourceHeight =
            (parentHeight - (topPx + bottomPx)) / getUnitSize(l.targetHeightUnit, true);
      } else {
        // +bottom
        l.setTargetBottom = true;
        l.sourceBottom =
            (parentHeight - (topPx + heightPx)) / getUnitSize(l.targetBottomUnit, true);
      }
   ...
   *
   * <p>Each child element has three values associated with each axis: {left, right, width} on the
   ...
   *
   * <ul>
   *   <li>{@link Layer#setLeftRight}
   *   <li>{@link Layer#setLeftWidth}
   *   <li>{@link Layer#setRightWidth}
   *   <li>{@link Layer#setTopBottom}
   *   <li>{@link Layer#setTopHeight}
   *   <li>{@link Layer#setBottomHeight}
   * </ul>
   *
   * <p>By default, each layer is set to fill the entire parent (i.e., {left, top, right, bottom} =
   * {0, 0, 0, 0}).
   */
  public class Layer {
     ...
     *
     * <p>This is the element that sits between the parent and child elements. It is normally
     * necessary to operate on this element only when you need to modify CSS properties that are not
     * directly modeled by the Layer class.
     *
